...
if(empty($post_title)||empty($post_category_id) ||empty($post_author) ||empty($post_image) ||empty($image_tmp) ||empty($post_tag) ||empty($post_date) ||empty($post_comment)||empty($post_content) ||empty($post_status) ){
	echo "<div class='alert alert-success' Role='alert'> Post Failed: post fields are empty. kindly fill it</div>";
}else{
		move_uploaded_file($image_tmp, "../images/".$post_image);
		

		$sql = "INSERT INTO post_table (post_title, post_author, post_date, post_image, post_content, post_tag, post_status, post_category_id, post_comment_count)VALUES('$post_title', 
		'$post_author', '$post_date', '$post_image', '$post_content', '$post_tag', '$post_status', '$post_category_id', '$post_comment')";
...
<label for="post_date">Post Status</label>
	...
	<div class="form-group">
	    ...
	    <textarea class="form-control" id="" name="post_comment" rows="3"></textarea>
  </div>

	<div class="form-group">
	    <label for="post_content">Post Content</label>
	    <textarea class="form-control" id="editor" name="post_content" rows="3"></textarea>
  </div>
